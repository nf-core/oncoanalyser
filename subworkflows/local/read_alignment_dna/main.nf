...
workflow READ_ALIGNMENT_DNA {
    ...
    ch_inputs_tumor_sorted = ch_inputs
        .branch { meta ->
            def has_existing = Inputs.hasExistingInput(meta, SampleMeta.INPUT.BAM_DNA_TUMOR)
    ...
    ch_inputs_normal_sorted = ch_inputs
        .branch { meta ->
            def has_existing = Inputs.hasExistingInput(meta, SampleMeta.INPUT.BAM_DNA_NORMAL)
    ...
    ch_inputs_donor_sorted = ch_inputs
        .branch { meta ->
            def has_existing = Inputs.hasExistingInput(meta, SampleMeta.INPUT.BAM_DNA_DONOR)
    ...
    ch_bam_tumor_out = Channel.empty()
        .mix(
            WorkflowChannels.restoreMeta(ch_bams_united.tumor, ch_inputs),
            PlaceholderChannels.bamBai(ch_inputs_tumor_sorted.skip),
        )

    ch_bam_normal_out = Channel.empty()
        .mix(
            WorkflowChannels.restoreMeta(ch_bams_united.normal, ch_inputs),
            PlaceholderChannels.bamBai(ch_inputs_normal_sorted.skip),
        )

    ch_bam_donor_out = Channel.empty()
        .mix(
            WorkflowChannels.restoreMeta(ch_bams_united.donor, ch_inputs),
