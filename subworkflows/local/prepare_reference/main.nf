...
include { CUSTOM_EXTRACTTARBALL as DECOMP_MINIBWA_INDEX } from '../../../modules/local/custom/extract_tarball/main'
...
include { WRITE_REFERENCE_DATA as WRITE_MINIBWA_INDEX    } from '../../../modules/local/custom/write_reference_data/main'
...
workflow PREPARE_REFERENCE {
    ...
    //
    // Set minibwa index, unpack or create if required
    //
    ch_genome_minibwa_index = channel.empty()
    if (prep_config.require_minibwa_index) {

        if (! params.ref_data_genome_minibwa_index) {

            MINIBWA_INDEX(
                ch_genome_fasta,
            )
            ch_genome_minibwa_index = channel.topic('minibwa_index')

        } else if (params.ref_data_genome_minibwa_index.endsWith('.tar.gz')) {

            ch_genome_minibwa_index_inputs = channel.of(params.ref_data_genome_minibwa_index)
                .map { fp_str -> def fp = file(fp_str); return [[topic_key: fp_str, id: "${fp.name.replaceAll('\\.tar\\.gz\$', '')}"], fp] }

            DECOMP_MINIBWA_INDEX(ch_genome_minibwa_index_inputs)
            ch_genome_minibwa_index = channel.topic('extracted_dir')
                .filter { meta, _dir -> meta.topic_key == params.ref_data_genome_minibwa_index }
                .map { _meta, dir -> dir }

        } else {

            ch_genome_minibwa_index = channel.fromPath(params.ref_data_genome_minibwa_index)
    ...
    if (prep_config.prepare_ref_data_only || params.prepare_reference_only) {
        ...
        WRITE_MINIBWA_INDEX(ch_genome_minibwa_index)
    ...
    genome_minibwa_index = ch_genome_minibwa_index.first() // path: genome_minibwa_index
