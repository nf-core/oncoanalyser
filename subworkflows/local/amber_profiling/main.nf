    ...
    ch_inputs_sorted = WorkflowChannels.groupByMeta(
        ...
        .map { meta, tumor_bam, tumor_bai, normal_bam, normal_bai, donor_bam, donor_bai ->
            return [
                ...
                Inputs.preferUserProvidedInput(donor_bam, meta, Inputs.KEY.BAM_REDUX_DNA_DONOR),
                Inputs.preferPipelineOutput(donor_bai, meta, Inputs.KEY.BAI_DNA_DONOR),
            ]
        }
        .branch { meta, tumor_bam, tumor_bai, normal_bam, normal_bai, donor_bam, donor_bai ->
            def has_existing = Inputs.hasExistingInput(meta, Inputs.KEY.AMBER_DIR)
