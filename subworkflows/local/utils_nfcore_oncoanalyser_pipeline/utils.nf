...
def parseInput(input_fp_str, stub_run, log) {
    ...
    def inputs = grouped_entries.collect { group_id, entries ->
            ...
            entries.each {
                ...
                // Sample type
                def sample_type_enum = getEnumFromString(it.sample_type, Constants.SampleType)
                if (! sample_type_enum) {
                    def sample_type_str = getEnumNames(Constants.SampleType).join('\n  - ')
                    log.error "received invalid sample type: '${it.sample_type}'. Valid options are:\n  - ${sample_type_str}"
                    exit 1
                }

                // Sequence type
                def sequence_type_enum = getEnumFromString(it.sequence_type, Constants.SequenceType)
                if (! sequence_type_enum) {
                    def sequence_type_str = getEnumNames(Constants.SequenceType).join('\n  - ')
                    log.error "received invalid sequence type: '${it.sequence_type}'. Valid options are:\n  - ${sequence_type_str}"
                    exit 1
                }

                // Filetype
                def filetype_enum = getEnumFromString(it.filetype, Constants.FileType)
                if (! filetype_enum) {
                    def filetype_str = getEnumNames(Constants.FileType).join('\n  - ')
                ...
                if (it.containsKey('info')) {
                    ...
                    it.info
                        ...
                        .each { e ->
                            ...
                            def info_field_enum = getEnumFromString(k, Constants.InfoField)

                            if (! info_field_enum) {
                                def info_field_str = getEnumNames(Constants.InfoField).join('\n  - ')
