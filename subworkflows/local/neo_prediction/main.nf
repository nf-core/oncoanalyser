...
workflow NEO_PREDICTION {
    ...
    ch_finder_inputs_selected = channels.WorkflowChannels.groupByMeta(
        ch_purple,
        ch_linx,
    )
        .map { meta, purple_dir, linx_annotation_dir ->
            ...
            def inputs = [
                ...
                Inputs.preferUserProvidedInput(linx_annotation_dir, meta, Inputs.KEY.LINX_ANNO_DIR_TUMOR),
            ]
    ...
    ch_finder_out = channels.WorkflowChannels.restoreMeta(NEO_FINDER.out.neo_finder_dir, ch_inputs)
    ...
    // channel: skip: [ meta ]
    ch_isofox_inputs_sorted = channels.WorkflowChannels.groupByMeta(
        ch_finder_out,
        ch_tumor_rna_bam,
    )
        .map { meta, neo_finder_dir, tumor_bam, tumor_bai ->
            return [
                meta,
                neo_finder_dir,
                Inputs.preferUserProvidedInput(tumor_bam, meta, Inputs.KEY.BAM_RNA_TUMOR),
                Inputs.preferUserProvidedInput(tumor_bai, meta, Inputs.KEY.BAI_RNA_TUMOR),
            ]
        }
    ...
    ch_annotate_fusions_out = Channel.empty()
        .mix(
            channels.WorkflowChannels.restoreMeta(NEO_ANNOTATE_FUSIONS.out.annotated_fusions, ch_inputs),
            channels.PlaceholderChannels.toolDir(ch_isofox_inputs_sorted.skip),
        )
    ...
    ch_scorer_inputs = channels.WorkflowChannels.groupByMeta(
        ch_isofox,
        ch_purple,
        ...
        .map { meta, isofox_dir, purple_dir, sage_somatic_append, lilac_dir, neo_finder_dir, annotated_fusions ->
            ...
            if (Inputs.hasTumorRna(meta)) {
                ...
                def sage_somatic_append_selected = Inputs.preferUserProvidedInput(sage_somatic_append, meta, Inputs.KEY.SAGE_APPEND_DIR_TUMOR)
                sage_somatic_append_vcf = file(sage_somatic_append_selected).resolve("${meta_scorer.sample_id}.sage.append.vcf.gz")
            }

            def inputs = [
                Inputs.preferUserProvidedInput(isofox_dir, meta, Inputs.KEY.ISOFOX_DIR),
                Inputs.preferUserProvidedInput(purple_dir, meta, Inputs.KEY.PURPLE_DIR),
                sage_somatic_append_vcf,
                Inputs.preferUserProvidedInput(lilac_dir, meta, Inputs.KEY.LILAC_DIR),
                neo_finder_dir,
                annotated_fusions,
            ]
