    ...
    ch_finder_inputs_selected = WorkflowChannels.groupByMeta(
        ...
        .map { meta, purple_dir, linx_annotation_dir ->
            ...
            def inputs = [
                ...
                Inputs.preferUserProvidedInput(linx_annotation_dir, meta, SampleSheetFields.INPUT.LINX_ANNO_DIR_TUMOR),
            ]
    ...
    ch_scorer_inputs = WorkflowChannels.groupByMeta(
        ...
        .map { meta, isofox_dir, purple_dir, sage_somatic_append, lilac_dir, neo_finder_dir, annotated_fusions ->
            ...
            if (Inputs.hasTumorRna(meta)) {
                ...
                def sage_somatic_append_selected = Inputs.preferUserProvidedInput(sage_somatic_append, meta, SampleSheetFields.INPUT.SAGE_APPEND_DIR_TUMOR)
                sage_somatic_append_vcf = file(sage_somatic_append_selected).resolve("${meta_scorer.sample_id}.sage.append.vcf.gz")
            }

            def inputs = [
                Inputs.preferUserProvidedInput(isofox_dir, meta, SampleSheetFields.INPUT.ISOFOX_DIR),
                Inputs.preferUserProvidedInput(purple_dir, meta, SampleSheetFields.INPUT.PURPLE_DIR),
                sage_somatic_append_vcf,
                Inputs.preferUserProvidedInput(lilac_dir, meta, SampleSheetFields.INPUT.LILAC_DIR),
                neo_finder_dir,
                annotated_fusions,
            ]
