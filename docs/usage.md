...
Then run `oncoanalyser` providing:
- `--mode purity_estimate`
- `--purity_estimate_mode <wgts|targeted>`
- (Optional) Panel reference data configuration for `--purity_estimate_mode targeted`
...
more processes (nf-core documentation: [tuning workflow resources](https://nf-co.re/docs/usage/configuration#tuning-workflow-resources)). For example, for high depth samples (e.g. panel samples), you may need increase the memory for 
alignment, read processing (REDUX) and/or variant calling (SAGE or ESVEE) steps.

Below are some recommended settings per WiGiTS tool from which you begin customising compute resource allocations:

```groovy
process {
    withName: '.*ALIGN'        { memory = 72.GB; cpus = 12; }
    ...
    withName: 'TEAL.*'         { memory = 32.GB; cpus = 32; }
    withName: VIRUSBREAKEND    { memory = 64.GB; cpus = 16; }
    withName: VIRUSINTERPRETER { memory = 8.GB;  cpus = 2;  }
}
```

Lastly, we recommend setting an upper limit on total resources that `oncoanalyser` is allowed to use (nf-core 
